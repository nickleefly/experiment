...
#ifdef DEBUG
/* A function to produce a rough output of the 2-3 tree.  Also includes
 * debugging checks.
 */
void *tree23_dump_r(tree23_node_t *p, int l)
{
    ...
    spaces(2*l);
    if(p->key_item1) {
	printf("(%d)", ((test_item_t *)p->key_item1)->data1);
    }
    else {
	printf("(--)");
    }
    if(p->key_item2) {
	printf("(%d)", ((test_item_t *)p->key_item2)->data1);
    }
    else {
	printf("(--)");
    }
	...
	putchar('\n');
        ...
        if(p->middle.item != p->key_item1 || p->right.item != p->key_item2) {
